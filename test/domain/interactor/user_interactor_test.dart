...
void main() {
  group('UserInteractor initialization logic', () {
    registerFallbackValue(
      UserData(0, 'Test', 'Test', 'Test', UserStatus.employee, 'test',
    ...
    test('Load initial list when UserDataSource empty', () async {
      ...
      var initialUserList = [
        UserData(
            0, 'Test', 'Test', 'Test', UserStatus.employee, 'test', 'test'),
        UserData(
          1,
          'Test',
          'Test',
    ...
    test('Fetch initial list not called when datasource contains users',
        () async {
      var initialUserList = [
        UserData(
            0, 'Test', 'Test', 'Test', UserStatus.employee, 'test', 'test'),
        UserData(
            1, 'Test', 'Test', 'Test', UserStatus.employee, 'test', "test"),
      ];
      var uds = UserDataSourceMock();
      when(() => uds.fetchUserDataList()).thenAnswer(
          (invocation) => Stream<List<UserData>>.value(initialUserList));
